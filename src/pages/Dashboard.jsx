...
const TABS = [
  ...
  { key: 'saved',        label: 'Saved',         icon: 'M17.593 3.322c1.1.128 1.907 1.077 1.907 2.185V21L12 17.25 4.5 21V5.507c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0111.186 0z' },
...
function DreamJourneyTracker({ status }) {
  ...
  return (
    <div className="flex items-center w-full mt-4 pt-4 border-t border-slate-100">
      {JOURNEY.map((s, i) => (
        <div key={s.key} className="flex items-center flex-1 min-w-0">
          <div className="flex flex-col items-center gap-1 flex-shrink-0">
            <div className={`w-2.5 h-2.5 rounded-full transition-all ${i <= current ? s.color : 'bg-slate-200'}`} />
            <span className={`text-[9px] font-semibold leading-none text-center ${i <= current ? 'text-slate-700' : 'text-slate-400'}`}
              style={{ maxWidth: 52 }}>
              {s.label}
            </span>
          </div>
          {i < JOURNEY.length - 1 && (
            <div className={`flex-1 h-px mx-1 mb-4 ${i < current ? s.color.replace('bg-', 'bg-') : 'bg-slate-200'}`} />
          )}
        </div>
      ))}
    </div>
...
function StatCard({ label, value, sub, icon, color = 'blue' }) {
  const colors = {
    blue:    { bg: 'bg-white', accent: 'bg-blue-50 text-blue-600',    text: 'text-blue-600',    border: 'border-slate-100' },
    emerald: { bg: 'bg-white', accent: 'bg-emerald-50 text-emerald-600', text: 'text-emerald-600', border: 'border-slate-100' },
    violet:  { bg: 'bg-white', accent: 'bg-violet-50 text-violet-600',   text: 'text-violet-600',  border: 'border-slate-100' },
    amber:   { bg: 'bg-white', accent: 'bg-amber-50 text-amber-600',     text: 'text-amber-600',   border: 'border-slate-100' },
  };
  const c = colors[color];
  return (
    <div className={`${c.bg} rounded-2xl border ${c.border} shadow-card p-5`}>
      <div className={`w-9 h-9 rounded-xl flex items-center justify-center mb-4 ${c.accent}`}>
        {icon}
      </div>
...
export default function Dashboard() {
  ...
  return (
    /*
     * pt-16 = navbar height (64 px, fixed). This single offset clears the
     * fixed navbar for all content beneath it — no per-section guesswork.
     */
    <div className="min-h-screen bg-slate-50 pt-16">
      <Navbar />

      {/* ── Dashboard Tab Bar ─────────────────────────────────────────
          sticky top-16 (z-40) = glues just below the fixed navbar (z-50).
          Uses border-b-2 underline pattern (Stripe/Notion style) rather
          than pill cards so it reads as navigation, not a content block.
      ─────────────────────────────────────────────────────────────── */}
      <div className="sticky top-16 z-40 bg-white/95 backdrop-blur-xl border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="flex items-stretch overflow-x-auto scrollbar-hide">

            {TABS.map((t) => (
              <button key={t.key} onClick={() => setTab(t.key)}
                className={`group relative flex items-center gap-2 px-4 py-3.5 text-sm font-semibold whitespace-nowrap transition-colors flex-shrink-0
                  ${tab === t.key
                    ? 'text-blue-600'
            ...
            {/* Spacer + CTA — right-anchored */}
            <div className="ml-auto flex items-center gap-2 py-2 pl-4 flex-shrink-0 border-l border-slate-100 ml-4">
      ...
      {/* ── Profile Hero Banner ───────────────────────────────────────
          Compact identity strip — name, role, location + three quick
          stats. No SVG ring (removed clutter). Sits immediately below
          the tab bar in the document flow.
      ─────────────────────────────────────────────────────────────── */}
      <div className="gradient-mesh relative overflow-hidden">
        <div className="absolute inset-0 dot-pattern opacity-20 pointer-events-none" />
        <div className="max-w-7xl mx-auto px-6 lg:px-8 py-7 relative">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5">

            {/* Identity */}
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center text-white text-lg font-black shadow-blue flex-shrink-0">
                {profile.name?.[0]?.toUpperCase()}
              </div>
              <div>
                <div className="flex items-center gap-2 mb-0.5">
                  <h1 className="text-lg font-black text-white leading-tight">{profile.name}</h1>
                  {profile.verified === 1 && (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-emerald-500/20 text-emerald-400 text-[10px] font-bold rounded-full border border-emerald-500/30">
                ...
                <p className="text-slate-400 text-xs font-medium capitalize">
                  {profile.mode} · {profile.location || 'Location not set'}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              {[
                { label: 'Trust',     value: profile.trust_score || 100 },
                { label: 'Dreams',    value: dreams.length },
                { label: 'Fulfilled', value: profile.fulfilled_count || 0 },
              ].map(({ label, value }) => (
                <div key={label} className="glass-dark rounded-xl px-4 py-2.5 text-center min-w-[64px] border border-white/[0.08]">
                  <div className="text-lg font-black text-white leading-none">{value}</div>
                  <div className="text-slate-400 text-[10px] font-semibold uppercase tracking-wider mt-0.5">{label}</div>
                </div>
              ))}
            </div>
      ...
      {/* ── Main Content ─────────────────────────────────────────────── */}
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-8 pb-20">

        {/* Stats row — no negative margin, clean top padding from hero */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <StatCard label="Active Dreams" value={activeCount} color="blue" icon={
          ...
          <StatCard label="Applications" value={fulfillments.length} color="violet" icon={
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <AnimatePresence mode="wait">
          ...
          {tab === 'dreams' && (
            <motion.div key="dreams" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              <div className="flex items-center justify-between mb-5">
                <div>
                  <h2 className="font-bold text-slate-900 text-lg">Dream Journey</h2>
                  <p className="text-slate-500 text-sm mt-0.5">Track every dream from submission to fulfillment</p>
                </div>
                {/* Mobile-only new dream button (desktop has it in the tab bar) */}
                <Link to="/submit-dream"
                  className="flex lg:hidden items-center gap-1.5 px-3.5 py-2 bg-blue-600 text-white text-xs font-bold rounded-xl hover:bg-blue-700 transition-colors shadow-sm">
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 4v16m8-8H4"/>
                  </svg>
                  New Dream
              ...
              {loading ? (
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-card p-16 text-center">
                  ...
                  <p className="font-bold text-slate-900 mb-1.5">No dreams submitted yet</p>
                  <p className="text-slate-500 text-sm mb-6 max-w-xs mx-auto leading-relaxed">
                    Share your dream anonymously and let the right person find you.
                  </p>
                  <Link to="/submit-dream"
                    className="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 text-white font-bold rounded-2xl hover:bg-blue-700 transition-colors shadow-blue text-sm">
                    ...
                    <motion.div key={dream.id} layout
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-wrap items-center gap-2 mb-2">
                            <h3 className="font-bold text-slate-900 leading-tight">{dream.title}</h3>
                            {dream.badge && dream.badge !== 'none' && BADGE_CONFIG[dream.badge] && (
                              <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${BADGE_CONFIG[dream.badge].color}`}>
                      ...
                      {dream.status === 'matched' && (
                        <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between">
                          <span className="text-sm text-slate-600 font-medium">A fulfiller has been matched to your dream</span>
          ...
          {tab === 'fulfillments' && (
            <motion.div key="fulfillments" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              <div className="mb-5">
                <h2 className="font-bold text-slate-900 text-lg">Fulfillment Applications</h2>
                <p className="text-slate-500 text-sm mt-0.5">Dreams you have applied to fulfill</p>
              </div>
              ...
              {fulfillments.length === 0 ? (
                <div className="bg-white rounded-2xl border border-slate-100 shadow-card p-16 text-center">
                  ...
                  <p className="font-bold text-slate-900 mb-1.5">No applications yet</p>
                  <p className="text-slate-500 text-sm mb-6">Browse dreams and apply to help someone make their dream real.</p>
                  <Link to="/dreams" className="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 text-white font-bold rounded-2xl hover:bg-blue-700 transition-colors shadow-blue text-sm">
                    Explore Dreams
                  </Link>
                </div>
              ) : (
                    ...
                    <div key={req.id} className="bg-white rounded-2xl border border-slate-100 shadow-card p-6">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          ...
                          <h3 className="font-bold text-slate-900 mb-2 leading-tight">{req.dream_title}</h3>
          ...
          {tab === 'saved' && (
            <motion.div key="saved" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              <div className="mb-5">
                <h2 className="font-bold text-slate-900 text-lg">Saved Dreams</h2>
                <p className="text-slate-500 text-sm mt-0.5">Dreams you have bookmarked to follow or fulfill</p>
              </div>
              ...
              {saved.length === 0 ? (
                <div className="bg-white rounded-2xl border border-slate-100 shadow-card p-16 text-center">
                  ...
                  <p className="font-bold text-slate-900 mb-1.5">No saved dreams</p>
                    ...
                    <div key={item.id} className="bg-white rounded-2xl border border-slate-100 shadow-card p-5">
                      <div className="flex items-start justify-between gap-3 mb-4">
                        <div className="flex-1 min-w-0">
                          ...
                          <h3 className="font-bold text-slate-900 mt-2 leading-snug">{item.title}</h3>
                        </div>
                        <button onClick={() => handleUnsave(item.dream_id)}
                          className="flex-shrink-0 w-8 h-8 rounded-xl bg-slate-100 text-slate-400 hover:bg-red-50 hover:text-red-500 flex items-center justify-center transition-colors">
          ...
          {tab === 'profile' && profileForm && (
            <motion.div key="profile" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              <div className="mb-5">
                <h2 className="font-bold text-slate-900 text-lg">My Profile</h2>
                <p className="text-slate-500 text-sm mt-0.5">Manage your identity, mode, and fulfiller bio</p>
              </div>
                  ...
                  <div className="bg-white rounded-2xl border border-slate-100 shadow-card p-6">
                    ...
                    <div>
                      ...
                      <textarea value={profileForm.fulfiller_bio || ''} rows={4}
                        placeholder="Describe your skills, experience, and what kinds of dreams you'd like to fulfill..."
                ...
                <div className="space-y-4">
                  <div className="bg-white rounded-2xl border border-slate-100 shadow-card p-6">
                    ...
                    <div className="text-center py-3">
                      <div className="text-5xl font-black text-blue-600 mb-1">{profile.impact_score || 0}</div>
                      <p className="text-slate-500 text-xs">Points earned through platform activity</p>
                    </div>
